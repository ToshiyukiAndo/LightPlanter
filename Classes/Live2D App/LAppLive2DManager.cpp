...
{
	int no = (sceneIndex+1) % 3;    //登録キャラクター数
	changeScene(no);
}
...
void LAppLive2DManager::changeScene(int index)
{
	...
	switch (sceneIndex)
	{
		case SCENE_INDEX_HANA:
			releaseModel();
			models.push_back(new LAppModel());
			models[0]->load(MODEL_HANA_DIR, MODEL_HANA) ;
			break;
		case SCENE_INDEX_HANA2:
			releaseModel();
			models.push_back(new LAppModel());
			models[0]->load(MODEL_HANA2_DIR, MODEL_HANA2) ;
			break;
